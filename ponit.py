...
class ScreenShot():
    ...
    def captureImage(self):
        if self.area.empty():
            ...
            box_area = [ int(x * screen_scale_rate) for x in self.area.area_box.box()]
            # 更新坐标值
            config_dict['coordinates']['first_point'] = ','.join(map(str, box_area[:2]))
            config_dict['coordinates']['second_point'] = ','.join(map(str, box_area[2:]))
            config.clear()
